<html>
	...
	<body>
		...
		#características producto comestible
		$tid = $_GET['tid'];
		$pid = $_GET['pid'];
		$query = "SELECT Productos.nombre, Productos.descripción, Productos.precio, ProductosNoComestibles.ancho, ProductosNoComestibles.largo, ProductosNoComestibles.alto, ProductosNoComestibles.peso FROM Productos, ProductosNoComestibles WHERE Productos.pid = $pid AND ProductosNoComestibles.pid = Productos.pid";
		...
		<section class="hero">
			<div class="content">
				<table id = "v1" class="table">
					...
					<th class = "title has-text-white has-text-centered">Ancho</th>
					<th class = "title has-text-white has-text-centered">Largo</th>
					<th class = "title has-text-white has-text-centered">Alto</th>
					<th class = "title has-text-white has-text-centered">Peso</th>
					</thead>
					<?php
					foreach ($dataCollected as $p) {
					echo ' <tr> <td class = "has-text-centered">' .$pid .'</td><td class = "has-text-centered">' .$p[0] .'</td> <td class = "has-text-centered">' .$p[1] .'</td> <td class = "has-text-centered">' .$p[2] .'</td> <td class = "has-text-centered">' .$p[3] .'</td> <td class = "has-text-centered">' .$p[4] .'</td> <td class = "has-text-centered">' .$p[5] .'</td> <td class = "has-text-centered">' .$p[6] .'</td></tr> ';
					}
					?>
				</table>
			</div>
